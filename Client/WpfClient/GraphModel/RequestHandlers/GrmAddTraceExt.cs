...
namespace Fibertest.WpfClient
{
    public static class GrmAddTraceExt
    {
        ...
        {
            if (! await model.Validate(request))
                return;
            ...
            if (fiberIds.Count + 1 != traceNodes.Count)
            {
                ...
                await model.WindowManager.ShowDialogWithAssignedOwner(errVm);
                return;
            }
            ...
            try
            {
                ...
                await model.WindowManager.ShowDialogWithAssignedOwner(vm);
                ...
                await model.WindowManager.ShowDialogWithAssignedOwner(traceAddViewModel);
        ...
        private static async Task<bool> Validate(this GraphReadModel model, RequestAddTrace request)
        {
            if (model.ReadModel.Equipments.Any(e => e.NodeId == request.LastNodeId && e.Type > EquipmentType.CableReserve)) return true;

            await model.WindowManager.ShowDialogWithAssignedOwner(new MyMessageBoxViewModel(MessageType.Error, Resources.SID_Last_node_of_trace_must_contain_some_equipment));
            return false;
        }
        ...
        private static async Task<List<Guid>?> GetPath(this GraphReadModel graphReadModel, RequestAddTrace request)
        {
            ...
            await graphReadModel.WindowManager.ShowWindowWithAssignedOwner(vm);

            var pathFinder = new PathFinder(graphReadModel.ReadModel);
            var path = await Task.Factory.StartNew(() => 
                pathFinder.FindPath(request.NodeWithRtuId, request.LastNodeId).Result);
            ...
            if (path == null)
            {
                ...
                await graphReadModel.WindowManager.ShowDialogWithAssignedOwner(new MyMessageBoxViewModel(MessageType.Error, strs));
            }
