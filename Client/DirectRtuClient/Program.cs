...
namespace Fibertest.DirectRtuClient
{
    internal class Program
    {
        static async Task Main()
        {
            ...
            var command = new DcGrpcCommand() { Json = JsonConvert.SerializeObject(dto, jsonSerializerSettings) };
            DcGrpcResponse response = await grpcClient.SendCommandAsync(command);
