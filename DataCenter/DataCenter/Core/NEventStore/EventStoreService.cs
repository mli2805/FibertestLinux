...
public class EventStoreService
{
    ...
    public async Task InitializeBothDb()
    {
        ...
        if (resetDb)
        {
            _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), "ResetDb flag is TRUE! DB will be deleted...");
            ...
            _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), "Db deleted successfully.");
        }
        else
            StreamIdOriginal = _dbInitializer.GetStreamIdIfExists();

        if (StreamIdOriginal != Guid.Empty)
            _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), $"Found DB with StreamIdOriginal {StreamIdOriginal}");
        else
        {
            StreamIdOriginal = Guid.NewGuid();
            _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), $"DB will be created with StreamIdOriginal {StreamIdOriginal}");
        }

        await using (var dbContext = new FtDbContext(_dbInitializer.FtDbContextOptions))
        {
            await dbContext.Database.EnsureCreatedAsync();
            _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), $"{_dbInitializer.ConnectionLogLine}");
        }
        var eventCount = await InitializeEventStoreService();
        _logger.Log(LogLevel.Information, Logs.DataCenter.ToInt(), $"Event store service initialization: {eventCount} events");
    }
