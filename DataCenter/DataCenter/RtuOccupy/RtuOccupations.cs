...
public class RtuOccupations
{
    ...
    public bool TrySetOccupation(Guid rtuId, RtuOccupation newRtuOccupation, string userName, out RtuOccupationState? state)
    {
        ...
        _logger.LLog(Logs.DataCenter.ToInt(), $@"Client {userName} asked to {action}");
        if (newRtuOccupation == RtuOccupation.None)
        {
            /////////////  it is a CHECK or CLEANUP  //////////////////////
            if (!RtuStates.TryGetValue(rtuId, out state))
            {
                _logger.LLog(Logs.DataCenter.ToInt(), $@"RTU {rtuId.First6()} is free already");
                return true;
            }
            ...
            {
                _logger.LLog(Logs.DataCenter.ToInt(), $@"RTU {rtuId.First6()} is free now");
                return true;
            }

            _logger.LLog(Logs.DataCenter.ToInt(), @"Something went wrong while dictionary cleanup!");
            return false;
        }
        ...
        { ////////// NEW OCCUPATION /////////////////
            ...
            _logger.LLog(Logs.DataCenter.ToInt(), @"Something went wrong while dictionary addition!");
            return false;
        }
        ...
        {  /////////  REFRESH   //////////////////
            ...
            _logger.LLog(Logs.DataCenter.ToInt(), @"Something went wrong while dictionary update!");
            return false;
        }
