...
public partial class RtuManager
{
    private async void RunMonitoringCycle()
    {
        ...
        _logger.LLog(Logs.RtuManager.ToInt(), Environment.NewLine + "Start monitoring.");
        _logger.LLog(Logs.RtuManager.ToInt(), $"_mainCharon.Serial = {_mainCharon.Serial}");

        if (_monitoringQueue.Count() < 1)
        {
            _logger.LLog(Logs.RtuManager.ToInt(), "There are no ports in queue for monitoring.");
        ...
        _logger.LLog(Logs.RtuManager.ToInt(), "Monitoring stopped.");

        _monitoringConfig.Update(c => c.IsMonitoringOn = false);
        await _otdrManager.DisconnectOtdr();
        _logger.LLog(Logs.RtuManager.ToInt(), "Rtu is turned into MANUAL mode.");
    }
    ...
    private async Task<MoniResult> DoFastMeasurement(MonitoringPort monitoringPort)
    {
        ...
        if (moniResult.MeasurementResult == MeasurementResult.Success)
        {
            ...
            if (message != "")
            {
                _logger.LLog(Logs.RtuManager.ToInt(), "Send by MSMQ:  " + message);
    ...
    private async Task<MoniResult> DoMeasurement(BaseRefType baseRefType, MonitoringPort monitoringPort, bool shouldChangePort = true)
    {
        ...
        _logger.LLog(Logs.RtuManager.ToInt(), $"Measurement result ({buffer.Length} bytes).");
        ...
        try
        {
            ...
            {
                _logger.LLog(Logs.RtuManager.ToInt(), "Additional check after measurement failed!");
        ...
        {
            _logger.LLog(Logs.RtuManager.ToInt(), $"Exception during PrepareMeasurement: {e.Message}");
        }

        MoniResult moniResult;
        try
        {
            _logger.LLog(Logs.RtuManager.ToInt(), "Start auto analysis.");
            ...
            _logger.LLog(Logs.RtuManager.ToInt(), $"Auto analysis applied. Now sor data has {measBytes.Length} bytes.");
        ...
        {
            _logger.LLog(Logs.RtuManager.ToInt(), $"Exception during measurement analysis: {e.Message}");
            return new MoniResult() { MeasurementResult = MeasurementResult.ComparisonFailed };
        }

        _logger.LLog(Logs.RtuManager.ToInt(), $"Trace state is {moniResult.GetAggregatedResult()}");
        foreach (var accidentInSor in moniResult.Accidents)
            _logger.LLog(Logs.RtuManager.ToInt(), accidentInSor.ToString());
        return moniResult;
    }
    ...
    private async Task ReInitializeDlls()
    {
        ...
        _logger.LLog(Logs.RtuManager.ToInt(), Environment.NewLine + $"OTDR initialization result - {otdrInitializationResult}");
        _logger.LLog(Logs.RtuService.ToInt(), Environment.NewLine + $"OTDR initialization result - {otdrInitializationResult}");
    }
